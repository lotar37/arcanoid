...
class Player:
    ...
    def move_avatar(self):
        mashtab = 50
        canvas.coords(self.avatar, self.x * mashtab, self.y * mashtab, self.x * mashtab \
                      + 2 * self.radius, self.y * mashtab + 2 * self.radius)
    ...
    def arrangement(self):
        for i in range(Game.trees):
            while True:
                ...
                if Game.playing_field[x][y] == "":
                    Game.playing_field[x][y] = "tree"
                    break
